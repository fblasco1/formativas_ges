...
def parsear_grupo(year: int, fase_text: str, grupo_text: str) -> Dict[str, str]:
    ...
    if year == 2019:
        ...
        return _parsear_grupo_2026(fase_text_upper, grupo_text_upper)
    else:
        return {
            "nivel": "Desconocido",
            "zona": "Desconocido",
...
def _parsear_grupo_2026(fase: str, grupo: str) -> Dict[str, str]:
    """
    2026: el nivel (CLASIFICACION LFF / RECLASIFICACION) sale de la fase (parsear_fase).
    Aquí solo desarmamos el combo Grupo (ej. NORTE 1A, CENTRO 2B, SUR 3C, ÚNICO).
    Devolvemos nivel=Desconocido para no pisar el nivel ya resuelto en merge_contexto_torneo.
    """
    nivel, zona, grupo_final = "Desconocido", "Desconocido", "Desconocido"
    g = grupo.upper().strip()
    g = g.replace("Ú", "U").replace("Ó", "O")

    if g in {"UNICO", "ÚNICO", "UNICA", "ÚNICA"}:
        return {"nivel": nivel, "zona": zona, "grupo": "UNICO"}

    # "NORTE 1A", "CENTRO 2B", "OESTE 3C" (también sin espacio extra)
    m = re.match(
        r"^(NORTE|CENTRO|SUR|OESTE)\s+(\d)\s*([A-Z])\s*$",
        g.replace("  ", " ").strip(),
    )
    if m:
        zona = m.group(1)
        grupo_final = f"{m.group(2)}{m.group(3)}"
        return {"nivel": nivel, "zona": zona, "grupo": grupo_final}

    # Fallback: última letra como subgrupo y prefijo como zona aproximada
    m2 = re.match(r"^([A-Z]+)\s+(\d+[A-Z])\s*$", g)
    if m2:
        zona = m2.group(1)
        grupo_final = m2.group(2)
        return {"nivel": nivel, "zona": zona, "grupo": grupo_final}

    grupo_final = grupo.strip() if grupo.strip() else "Desconocido"
    return {"nivel": nivel, "zona": zona, "grupo": grupo_final}